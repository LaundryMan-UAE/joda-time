...
#ifndef _OrgJodaTimePeriod_H_
#define _OrgJodaTimePeriod_H_
...
/**
 ...
 <p> The standard period type is used, thus you can add other fields such as months or days using the <code>withXxx()</code> methods. For example, <code>Period.years(2).withMonths(6);</code> <p> If you want a year-based period that cannot have other fields added, then you should consider using Years .
...
/**
 ...
 <p> The standard period type is used, thus you can add other fields such as years or days using the <code>withXxx()</code> methods. For example, <code>Period.months(2).withDays(6);</code> <p> If you want a month-based period that cannot have other fields added, then you should consider using Months .
...
/**
 ...
 <p> The standard period type is used, thus you can add other fields such as months or days using the <code>withXxx()</code> methods. For example, <code>Period.weeks(2).withDays(6);</code> <p> If you want a week-based period that cannot have other fields added, then you should consider using Weeks .
...
/**
 ...
 <p> The standard period type is used, thus you can add other fields such as months or weeks using the <code>withXxx()</code> methods. For example, <code>Period.days(2).withHours(6);</code> <p> If you want a day-based period that cannot have other fields added, then you should consider using Days .
...
/**
 ...
 <p> The standard period type is used, thus you can add other fields such as months or days using the <code>withXxx()</code> methods. For example, <code>Period.hours(2).withMinutes(30);</code> <p> If you want a hour-based period that cannot have other fields added, then you should consider using Hours .
...
/**
 ...
 <p> The standard period type is used, thus you can add other fields such as days or hours using the <code>withXxx()</code> methods. For example, <code>Period.minutes(2).withSeconds(30);</code> <p> If you want a minute-based period that cannot have other fields added, then you should consider using Minutes .
...
/**
 ...
 <p> The standard period type is used, thus you can add other fields such as days or hours using the <code>withXxx()</code> methods. For example, <code>Period.seconds(2).withMillis(30);</code> <p> If you want a second-based period that cannot have other fields added, then you should consider using Seconds .
...
/**
 ...
 <p> The standard period type is used, thus you can add other fields such as days or hours using the <code>withXxx()</code> methods. For example, <code>Period.millis(20).withSeconds(30);</code>
...
/**
 @brief Creates a period from two partially specified times, calculating by field difference.
 <p> The two partials must contain the same fields, thus you can specify two <code>LocalDate</code> objects, or two <code>LocalTime</code> objects, but not one of each. Also, the partial may not contain overlapping fields, such as dayOfWeek and dayOfMonth. <p> Calculation by field difference works by extracting the difference one field at a time and not wrapping into other fields. Thus 2005-06-09/2007-04-12 will yield P1Y-2M3D. <p> For example, you have an event that always runs from the 27th of each month to the 2nd of the next month. If you calculate this period using a standard constructor, then you will get between P3D and P6D depending on the month. If you use this method, then you will get P1M-25D. This field-difference based period can be successfully applied to each month of the year to obtain the correct end date for a given start date.
...
/**
 ...
 <p> One way to initialise a period is as follows: <pre> Period = new Period().withYears(6).withMonths(3).withSeconds(23); </pre> Bear in mind that this creates four period instances in total, three of which are immediately discarded. The alterative is more efficient, but less readable: <pre> Period = new Period(6, 3, 0, 0, 0, 0, 23, 0); </pre> The following is also slightly less wasteful: <pre> Period = Period.years(6).withMonths(3).withSeconds(23); </pre>
 */
- (instancetype)init;

/**
 @brief Create a period from a set of field values using the standard set of fields.
 Note that the parameters specify the time fields hours, minutes, seconds and millis, not the date fields.
...
/**
 ...
 <p> There is usually little need to use this constructor. The period type is used primarily to define how to split an interval into a period. As this constructor already is split, the period type does no real work.
...
/**
 ...
 <p> The two partials must contain the same fields, thus you can specify two <code>LocalDate</code> objects, or two <code>LocalTime</code> objects, but not one of each. As these are Partial objects, time zones have no effect on the result. <p> The two partials must also both be contiguous - see DateTimeUtils#isContiguous(ReadablePartial) for a definition. Both <code>LocalDate</code> and <code>LocalTime</code> are contiguous. <p> Most calculations performed by this method have obvious results. The special case is where the calculation is from a "long" month to a "short" month. Here, the result favours increasing the months field rather than the days. For example, 2013-01-31 to 2013-02-28 is treated as one whole month. By contrast, 2013-01-31 to 2013-03-30 is treated as one month and 30 days (exposed as 4 weeks and 2 days). The results are explained by considering that the start date plus the calculated period result in the end date. <p> An alternative way of constructing a Period from two Partials is #fieldDifference(ReadablePartial,ReadablePartial) . That method handles all kinds of partials.
 @param start the start of the period, must not be null
 @param end the end of the period, must not be null
 @throws IllegalArgumentException if the partials are null or invalid
 @since 1.1
...
/**
 @brief Creates a period from two partially specified times.
 <p> The two partials must contain the same fields, thus you can specify two <code>LocalDate</code> objects, or two <code>LocalTime</code> objects, but not one of each. As these are Partial objects, time zones have no effect on the result. <p> The two partials must also both be contiguous - see DateTimeUtils#isContiguous(ReadablePartial) for a definition. Both <code>LocalDate</code> and <code>LocalTime</code> are contiguous. <p> Most calculations performed by this method have obvious results. The special case is where the calculation is from a "long" month to a "short" month. Here, the result favours increasing the months field rather than the days. For example, 2013-01-31 to 2013-02-28 is treated as one whole month. By contrast, 2013-01-31 to 2013-03-30 is treated as one month and 30 days. The results are explained by considering that the start date plus the calculated period result in the end date. <p> An alternative way of constructing a Period from two Partials is #fieldDifference(ReadablePartial,ReadablePartial) . That method handles all kinds of partials.
 @param start the start of the period, must not be null
 @param end the end of the period, must not be null
 @param type which set of fields this period supports, null means standard
 @throws IllegalArgumentException if the partials are null or invalid
...
/**
 @brief Creates a new Period instance with the same field values but different PeriodType.
...
/**
 @brief Creates a new Period instance with the fields from the specified period copied on top of those from this period.
 <p> This period instance is immutable and unaffected by this method call.
...
/**
 ...
 <p> Each field of the period is added separately. Thus a period of 2 hours 30 minutes plus 3 hours 40 minutes will produce a result of 5 hours 70 minutes - see #normalizedStandard() . <p> If the period being added contains a non-zero amount for a field that is not supported in this period then an exception is thrown. <p> This period instance is immutable and unaffected by this method call.
...
/**
 ...
 <p> Each field of the period is subtracted separately. Thus a period of 3 hours 30 minutes minus 2 hours 40 minutes will produce a result of 1 hour and -10 minutes - see #normalizedStandard() . <p> If the period being added contains a non-zero amount for a field that is not supported in this period then an exception is thrown. <p> This period instance is immutable and unaffected by this method call.
 @param period the period to add, null adds zero and returns this
 @return the new updated period
 @throws UnsupportedOperationException if any field is not supported
 @since 1.5
...
/**
 @brief Returns a new instance with each element in this period multiplied by the specified scalar.
...
/**
 @brief Converts this period to a period in weeks assuming a 7 day week, 24 hour day, 60 minute hour and 60 second minute.
...
/**
 @brief Converts this period to a period in days assuming a 7 day week, 24 hour day, 60 minute hour and 60 second minute.
 <p> This method allows you to convert between different types of period. However to achieve this it makes the assumption that all weeks are 7 days, all days are 24 hours, all hours are 60 minutes and all minutes are 60 seconds. This is not true when daylight savings time is considered, and may also not be true for some unusual chronologies. However, it is included as it is a useful operation for many applications and business rules. <p> If the period contains years or months, an exception will be thrown.
...
/**
 @brief Converts this period to a period in hours assuming a 7 day week, 24 hour day, 60 minute hour and 60 second minute.
 <p> This method allows you to convert between different types of period. However to achieve this it makes the assumption that all weeks are 7 days, all days are 24 hours, all hours are 60 minutes and all minutes are 60 seconds. This is not true when daylight savings time is considered, and may also not be true for some unusual chronologies. However, it is included as it is a useful operation for many applications and business rules. <p> If the period contains years or months, an exception will be thrown.
...
/**
 @brief Converts this period to a period in minutes assuming a 7 day week, 24 hour day, 60 minute hour and 60 second minute.
 <p> This method allows you to convert between different types of period. However to achieve this it makes the assumption that all weeks are 7 days, all days are 24 hours, all hours are 60 minutes and all minutes are 60 seconds. This is not true when daylight savings time is considered, and may also not be true for some unusual chronologies. However, it is included as it is a useful operation for many applications and business rules. <p> If the period contains years or months, an exception will be thrown.
...
/**
 @brief Converts this period to a period in seconds assuming a 7 day week, 24 hour day, 60 minute hour and 60 second minute.
 <p> This method allows you to convert between different types of period. However to achieve this it makes the assumption that all weeks are 7 days, all days are 24 hours, all hours are 60 minutes and all minutes are 60 seconds. This is not true when daylight savings time is considered, and may also not be true for some unusual chronologies. However, it is included as it is a useful operation for many applications and business rules. <p> If the period contains years or months, an exception will be thrown.
...
/**
 @brief Converts this period to a duration assuming a 7 day week, 24 hour day, 60 minute hour and 60 second minute.
...
/**
 @brief Normalizes this period using standard rules, assuming a 12 month year, 7 day week, 24 hour day, 60 minute hour and 60 second minute, providing control over how the result is split into fields.
 ...
 @return a normalized period equivalent to this period
 @throws ArithmeticException if any field is too large to be represented
 @throws UnsupportedOperationException if this period contains non-zero years or months but the specified period type does not support them
 @since 1.5
 */
