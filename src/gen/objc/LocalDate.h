...
#ifndef _OrgJodaTimeLocalDate_H_
#define _OrgJodaTimeLocalDate_H_
...
/**
 @brief LocalDate is an immutable datetime class representing a date without a time zone.
 <p> LocalDate implements the ReadablePartial interface. To do this, the interface methods focus on the key fields - Year, MonthOfYear and DayOfMonth. However, <b>all</b> date fields may in fact be queried. <p> LocalDate differs from DateMidnight in that this class does not have a time zone and does not represent a single instant in time. <p> Calculations on LocalDate are performed using a Chronology . This chronology will be set internally to be in the UTC time zone for all calculations. <p>Each individual field can be queried in two ways: <ul> <li><code>getMonthOfYear()</code> <li><code>monthOfYear().get()</code> </ul> The second technique also provides access to other useful methods on the field: <ul> <li>numeric value <li>text value <li>short text value <li>maximum/minimum values <li>add/subtract <li>set <li>rounding </ul> <p> LocalDate is thread-safe and immutable, provided that the Chronology is as well. All standard Chronology classes supplied are thread-safe and immutable.
...
/**
 @brief Obtains a <code>LocalDate</code> set to the current system millisecond time using <code>ISOChronology</code> in the default time zone.
...
/**
 @brief Obtains a <code>LocalDate</code> set to the current system millisecond time using <code>ISOChronology</code> in the specified time zone.
...
/**
 @brief Obtains a <code>LocalDate</code> set to the current system millisecond time using the specified chronology.
...
/**
 @brief Constructs a LocalDate from a <code>java.util.Calendar</code> using exactly the same field values.
 <p> Each field is queried from the Calendar and assigned to the LocalDate. This is useful if you have been using the Calendar as a local date, ignoring the zone. <p> One advantage of this method is that this method is unaffected if the version of the time zone data differs between the JDK and Joda-Time. That is because the local field values are transferred, calculated using the JDK time zone data and without using the Joda-Time time zone data. <p> This factory method ignores the type of the calendar and always creates a LocalDate with ISO chronology. It is expected that you will only pass in instances of <code>GregorianCalendar</code> however this is not validated.
...
/**
 @brief Constructs a LocalDate from a <code>java.util.Date</code> using exactly the same field values.
...
/**
 ...
 The supported fields are Year, MonthOfYear and DayOfMonth. Note that all fields from day and above may in fact be queried via other methods.
...
/**
 ...
 <p> This method is required to support the <code>ReadablePartial</code> interface. The supported fields are Year, MonthOfYear and DayOfMonth. Note that all fields from day and above may in fact be queried via other methods.
...
/**
 ...
 <p> This method gets the value of the specified field. For example: <pre> LocalDate dt = LocalDate.nowDefaultZone(); int year = dt.get(DateTimeFieldType.year()); </pre>
...
/**
 @brief Checks if the field type specified is supported by this local date and chronology.
...
/**
 @brief Checks if the duration type specified is supported by this local date and chronology.
...
/**
 @brief Converts this LocalDate to a full datetime at the earliest valid time for the date using the default time zone.
...
/**
 @brief Converts this LocalDate to a full datetime at the earliest valid time for the date using the specified time zone.
...
/**
 @brief Converts this LocalDate to a full datetime at midnight using the default time zone.
 <p> This method will throw an exception if the default time zone switches to Daylight Savings Time at midnight and this LocalDate represents that switchover date. The problem is that there is no such time as midnight on the required date, and as such an exception is thrown. <p> This instance is immutable and unaffected by this method call.
 @return this date as a datetime at midnight
 */
- (OrgJodaTimeDateTime *)toDateTimeAtMidnight;

/**
 @brief Converts this LocalDate to a full datetime at midnight using the specified time zone.
 <p> This method will throw an exception if the time zone switches to Daylight Savings Time at midnight and this LocalDate represents that switchover date. The problem is that there is no such time as midnight on the required date, and as such an exception is thrown. <p> This method uses the chronology from this instance plus the time zone specified. <p> This instance is immutable and unaffected by this method call.
 @param zone the zone to use, null means default zone
 @return this date as a datetime at midnight
 */
...
/**
 @brief Converts this LocalDate to a full datetime using the default time zone setting the date fields from this instance and the time fields from the current time.
 <p> This method will throw an exception if the datetime that would be created does not exist when the time zone is taken into account. <p> This instance is immutable and unaffected by this method call.
 @return this date as a datetime with the time as the current time
 */
- (OrgJodaTimeDateTime *)toDateTimeAtCurrentTime;

/**
 @brief Converts this LocalDate to a full datetime using the specified time zone setting the date fields from this instance and the time fields from the current time.
 <p> This method uses the chronology from this instance plus the time zone specified. <p> This method will throw an exception if the datetime that would be created does not exist when the time zone is taken into account. <p> This instance is immutable and unaffected by this method call.
 @param zone the zone to use, null means default zone
 @return this date as a datetime with the time as the current time
 */
...
/**
 ...
 <p> As from v1.5, you are recommended to avoid DateMidnight and use #toDateTimeAtStartOfDay() instead because of the exception detailed below. <p> This method will throw an exception if the default time zone switches to Daylight Savings Time at midnight and this LocalDate represents that switchover date. The problem is that there is no such time as midnight on the required date, and as such an exception is thrown. <p> This instance is immutable and unaffected by this method call.
...
/**
 ...
 <p> As from v1.5, you are recommended to avoid DateMidnight and use #toDateTimeAtStartOfDay() instead because of the exception detailed below. <p> This method will throw an exception if the time zone switches to Daylight Savings Time at midnight and this LocalDate represents that switchover date. The problem is that there is no such time as midnight on the required date, and as such an exception is thrown. <p> This instance is immutable and unaffected by this method call.
...
/**
 @brief Converts this object to a LocalDateTime using a LocalTime to fill in the missing fields.
 <p> The resulting chronology is determined by the chronology of this LocalDate. The chronology of the time must also match. If the time is null an exception is thrown. <p> This instance is immutable and unaffected by this method call.
...
/**
 @brief Converts this object to a DateTime using a LocalTime to fill in the missing fields.
 <p> The resulting chronology is determined by the chronology of this LocalDate plus the time zone. The chronology of the time must match. <p> If the time is null, this method delegates to #toDateTimeAtCurrentTime(DateTimeZone) and the following documentation does not apply. <p> When the time zone is applied, the local date-time may be affected by daylight saving. In a daylight saving gap, when the local time does not exist, this method will throw an exception. In a daylight saving overlap, when the same local time occurs twice, this method returns the first occurrence of the local time. <p> This instance is immutable and unaffected by this method call.
 @param time the time of day to use, null uses current time
...
/**
 ...
 <p> The interval may have more or less than 24 hours if this is a daylight savings cutover date. <p> This instance is immutable and unaffected by this method call.
...
/**
 @brief Returns a copy of this date with the partial set of fields replacing those from this instance.
 <p> For example, if the partial contains a year and a month then those two fields will be changed in the returned instance. Unsupported fields are ignored. If the partial is null, then <code>this</code> is returned.
...
/**
 ...
 <p> For example, if the field type is <code>monthOfYear</code> then the month of year field will be changed in the returned instance. If the field type is null, then <code>this</code> is returned. <p> These two lines are equivalent: <pre> LocalDate updated = dt.withDayOfMonth(6); LocalDate updated = dt.withField(DateTimeFieldType.dayOfMonth(), 6); </pre>
...
/**
 ...
 <p> If the addition is zero or the field is null, then <code>this</code> is returned. <p> These three lines are equivalent: <pre> LocalDate added = dt.withFieldAdded(DurationFieldType.years(), 6); LocalDate added = dt.plusYears(6); LocalDate added = dt.plus(Period.years(6)); </pre>
...
/**
 @brief Returns a copy of this date with the specified period added.
 <p> If the amount is zero or null, then <code>this</code> is returned. <p> This method is typically used to add complex period instances. Adding one field is best achieved using methods like #plusYears(int) . <p> Unsupported time fields are ignored, thus adding a period of 24 hours will not have any effect.
 @param period the period to add to this one, null means zero
 @return a copy of this date with the period added
 @throws ArithmeticException if the result exceeds the internal capacity
 */
...
/**
 ...
 <p> This adds the specified number of months to the date. The addition may change the year, but the day-of-month is normally unchanged. If adding months makes the day-of-month invalid, it is adjusted to the last valid day in the month. This LocalDate instance is immutable and unaffected by this method call. <p> The following three lines are identical in effect: <pre> LocalDate added = dt.plusMonths(6); LocalDate added = dt.plus(Period.months(6)); LocalDate added = dt.withFieldAdded(DurationFieldType.months(), 6); </pre>
...
/**
 ...
 <p> This LocalDate instance is immutable and unaffected by this method call. <p> The following three lines are identical in effect: <pre> LocalDate added = dt.plusWeeks(6); LocalDate added = dt.plus(Period.weeks(6)); LocalDate added = dt.withFieldAdded(DurationFieldType.weeks(), 6); </pre>
...
/**
 ...
 <p> This LocalDate instance is immutable and unaffected by this method call. <p> The following three lines are identical in effect: <pre> LocalDate added = dt.plusDays(6); LocalDate added = dt.plus(Period.days(6)); LocalDate added = dt.withFieldAdded(DurationFieldType.days(), 6); </pre>
...
/**
 ...
 <p> If the amount is zero or null, then <code>this</code> is returned. <p> This method is typically used to subtract complex period instances. Subtracting one field is best achieved using methods like #minusYears(int) . <p> Unsupported time fields are ignored, thus subtracting a period of 24 hours will not have any effect.
...
/**
 ...
 <p> This subtracts the specified number of months from the date. The subtraction may change the year, but the day-of-month is normally unchanged. If subtracting months makes the day-of-month invalid, it is adjusted to the last valid day in the month. This LocalDate instance is immutable and unaffected by this method call. <p> The following three lines are identical in effect: <pre> LocalDate subtracted = dt.minusMonths(6); LocalDate subtracted = dt.minus(Period.months(6)); LocalDate subtracted = dt.withFieldAdded(DurationFieldType.months(), -6); </pre>
...
/**
 ...
 <p> This LocalDate instance is immutable and unaffected by this method call. <p> The following three lines are identical in effect: <pre> LocalDate subtracted = dt.minusWeeks(6); LocalDate subtracted = dt.minus(Period.weeks(6)); LocalDate subtracted = dt.withFieldAdded(DurationFieldType.weeks(), -6); </pre>
...
/**
 ...
 <p> This LocalDate instance is immutable and unaffected by this method call. <p> The following three lines are identical in effect: <pre> LocalDate subtracted = dt.minusDays(6); LocalDate subtracted = dt.minus(Period.days(6)); LocalDate subtracted = dt.withFieldAdded(DurationFieldType.days(), -6); </pre>
...
/**
 ...
 <p> LocalDate is immutable, so there are no set methods. Instead, this method returns a new instance with the value of era changed.
...
/**
 ...
 <p> LocalDate is immutable, so there are no set methods. Instead, this method returns a new instance with the value of century of era changed.
...
/**
 ...
 <p> LocalDate is immutable, so there are no set methods. Instead, this method returns a new instance with the value of year of era changed.
...
/**
 ...
 <p> LocalDate is immutable, so there are no set methods. Instead, this method returns a new instance with the value of year of century changed.
...
/**
 ...
 <p> LocalDate is immutable, so there are no set methods. Instead, this method returns a new instance with the value of year changed.
...
/**
 ...
 <p> LocalDate is immutable, so there are no set methods. Instead, this method returns a new instance with the value of month of year changed.
...
/**
 ...
 <p> LocalDate is immutable, so there are no set methods. Instead, this method returns a new instance with the value of day of year changed.
...
/**
 ...
 <p> LocalDate is immutable, so there are no set methods. Instead, this method returns a new instance with the value of day of month changed.
...
/**
 ...
 <p> LocalDate is immutable, so there are no set methods. Instead, this method returns a new instance with the value of day of week changed.
...
J2OBJC_STATIC_FIELD_GETTER(OrgJodaTimeLocalDate, serialVersionUID, long long int)

J2OBJC_STATIC_FIELD_GETTER(OrgJodaTimeLocalDate, YEAR, int)

J2OBJC_STATIC_FIELD_GETTER(OrgJodaTimeLocalDate, MONTH_OF_YEAR, int)

J2OBJC_STATIC_FIELD_GETTER(OrgJodaTimeLocalDate, DAY_OF_MONTH, int)
...
/**
 @brief LocalDate.Property binds a LocalDate to a DateTimeField allowing powerful datetime functionality to be easily accessed.
 <p> The simplest use of this class is as an alternative get method, here used to get the year '1972' (as an int) and the month 'December' (as a String). <pre> LocalDate dt = new LocalDate(1972, 12, 3, 0, 0); int year = dt.year().get(); String monthStr = dt.month().getAsText(); </pre> <p> Methods are also provided that allow date modification. These return new instances of LocalDate - they do not modify the original. The example below yields two independent immutable date objects 20 years apart. <pre> LocalDate dt = new LocalDate(1972, 12, 3); LocalDate dt1920 = dt.year().setCopy(1920); </pre> <p> LocalDate.Property itself is thread-safe and immutable, as well as the LocalDate being operated on.
 @author Stephen Colebourne
...
/**
 ...
 A field wrapped operation only changes this field. Thus 31st January addWrapField one day goes to the 1st January. <p> The LocalDate attached to this property is unchanged by this call.
 @param value the value to add to the field in the copy
 @return a copy of the LocalDate with the field value changed
 @throws IllegalArgumentException if the value isn't valid
 */
...
/**
 @brief Returns a new LocalDate with this field set to the maximum value for this field.
 <p> This operation is useful for obtaining a LocalDate on the last day of the month, as month lengths vary. <pre> LocalDate lastDayOfMonth = dt.dayOfMonth().withMaximumValue(); </pre> <p> The LocalDate attached to this property is unchanged by this call.
 @return a copy of the LocalDate with this field set to its maximum
 */
- (OrgJodaTimeLocalDate *)withMaximumValue;

/**
 @brief Returns a new LocalDate with this field set to the minimum value for this field.
 <p> The LocalDate attached to this property is unchanged by this call.
...
/**
 @brief Rounds to the lowest whole unit of this field on a copy of this LocalDate.
 <p> For example, rounding floor on the hourOfDay field of a LocalDate where the time is 10:30 would result in new LocalDate with the time of 10:00.
 @return a copy of the LocalDate with the field value changed
 */
- (OrgJodaTimeLocalDate *)roundFloorCopy;

/**
 @brief Rounds to the highest whole unit of this field on a copy of this LocalDate.
 <p> For example, rounding floor on the hourOfDay field of a LocalDate where the time is 10:30 would result in new LocalDate with the time of 11:00.
 @return a copy of the LocalDate with the field value changed
 */
- (OrgJodaTimeLocalDate *)roundCeilingCopy;

/**
 @brief Rounds to the nearest whole unit of this field on a copy of this LocalDate, favoring the floor if halfway.
 @return a copy of the LocalDate with the field value changed
 */
- (OrgJodaTimeLocalDate *)roundHalfFloorCopy;

/**
 @brief Rounds to the nearest whole unit of this field on a copy of this LocalDate, favoring the ceiling if halfway.
 @return a copy of the LocalDate with the field value changed
 */
- (OrgJodaTimeLocalDate *)roundHalfCeilingCopy;

/**
 @brief Rounds to the nearest whole unit of this field on a copy of this LocalDate.
...
J2OBJC_STATIC_FIELD_GETTER(OrgJodaTimeLocalDate_Property, serialVersionUID, long long int)
