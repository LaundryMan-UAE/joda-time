...
//  source: /Users/marcussmith/Laundryman/Libraries/joda-time/src/main/java/org/joda/time/field/DividedDateTimeField.java
//
...
inline jlong OrgJodaTimeFieldDividedDateTimeField_get_serialVersionUID(void);
...
- (jint)getWithLong:(jlong)instant {
  ...
  if (value >= 0) {
    return JreIntDiv(value, iDivisor_);
  }
  else {
    return (JreIntDiv((value + 1), iDivisor_)) - 1;
  }
}
...
- (jint)getDifferenceWithLong:(jlong)minuendInstant
  ...
  return JreIntDiv([((OrgJodaTimeDateTimeField *) nil_chk([self getWrappedField])) getDifferenceWithLong:minuendInstant withLong:subtrahendInstant], iDivisor_);
}

- (jlong)getDifferenceAsLongWithLong:(jlong)minuendInstant
                            withLong:(jlong)subtrahendInstant {
  return JreLongDiv([((OrgJodaTimeDateTimeField *) nil_chk([self getWrappedField])) getDifferenceAsLongWithLong:minuendInstant withLong:subtrahendInstant], iDivisor_);
}
...
void OrgJodaTimeFieldDividedDateTimeField_initWithOrgJodaTimeDateTimeField_withOrgJodaTimeDurationField_withOrgJodaTimeDateTimeFieldType_withInt_(OrgJodaTimeFieldDividedDateTimeField *self, OrgJodaTimeDateTimeField *field, OrgJodaTimeDurationField *rangeField, OrgJodaTimeDateTimeFieldType *type, jint divisor) {
  ...
  jint min = (i >= 0) ? JreIntDiv(i, divisor) : (JreIntDiv((i + 1), divisor) - 1);
  jint j = [field getMaximumValue];
  jint max = (j >= 0) ? JreIntDiv(j, divisor) : (JreIntDiv((j + 1), divisor) - 1);
  self->iMin_ = min;
  self->iMax_ = max;
}
...
jint OrgJodaTimeFieldDividedDateTimeField_getRemainderWithInt_(OrgJodaTimeFieldDividedDateTimeField *self, jint value) {
  if (value >= 0) {
    return JreIntMod(value, self->iDivisor_);
  }
  else {
    return (self->iDivisor_ - 1) + (JreIntMod((value + 1), self->iDivisor_));
  }
}
