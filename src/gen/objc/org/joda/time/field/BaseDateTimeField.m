...
- (IOSIntArray *)addWithOrgJodaTimeReadablePartial:(id<OrgJodaTimeReadablePartial>)instant
  ...
  while (valueToAdd > 0) {
    ...
    if (nextField == nil) {
      if (fieldIndex == 0) {
        @throw [new_JavaLangIllegalArgumentException_initWithNSString_(@"Maximum value exceeded for add") autorelease];
      }
      ...
      if ([((OrgJodaTimeDurationField *) nil_chk([self getRangeDurationField])) getType] != [((OrgJodaTimeDurationField *) nil_chk([((OrgJodaTimeDateTimeField *) nil_chk(nextField)) getDurationField])) getType]) {
        @throw [new_JavaLangIllegalArgumentException_initWithNSString_(@"Fields invalid for add") autorelease];
      }
    }
...
- (jint)convertTextWithNSString:(NSString *)text
  ...
  @catch (JavaLangNumberFormatException *ex) {
    @throw [new_OrgJodaTimeIllegalFieldValueException_initWithOrgJodaTimeDateTimeFieldType_withNSString_(OrgJodaTimeFieldBaseDateTimeField_getType(self), text) autorelease];
  }
}
