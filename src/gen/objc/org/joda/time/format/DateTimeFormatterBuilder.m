...
//  source: /Users/Salton/Documents/Projects/joda-time/src/main/java/org/joda/time/format/DateTimeFormatterBuilder.java
//
...
inline id<JavaUtilMap> OrgJodaTimeFormatDateTimeFormatterBuilder_TimeZoneId_get_GROUPED_IDS();
...
inline id<JavaUtilList> OrgJodaTimeFormatDateTimeFormatterBuilder_TimeZoneId_get_BASE_GROUPED_IDS();
...
- (jint)parseIntoWithOrgJodaTimeFormatDateTimeParserBucket:(OrgJodaTimeFormatDateTimeParserBucket *)bucket
  ...
  jint textLen = [((id<JavaLangCharSequence>) nil_chk(text)) length];
  ...
  for (jint i = pos; i < matchLen; i++) {
    if ([text charAtWithInt:i] == '/') {
      ...
      pos += ((jint) [((NSString *) nil_chk(prefix)) length]);
  ...
  for (jint i = 0; i < [((id<JavaUtilList>) nil_chk(suffixSet)) size]; i++) {
    ...
    if (OrgJodaTimeFormatDateTimeFormatterBuilder_csStartsWithWithJavaLangCharSequence_withInt_withNSString_(text, pos, suffix)) {
      if (best == nil || ((jint) [((NSString *) nil_chk(suffix)) length]) > ((jint) [best length])) {
        best = suffix;
      }
    }
  }
...
+ (void)initialize {
  if (self == [OrgJodaTimeFormatDateTimeFormatterBuilder_TimeZoneId class]) {
    ...
    for (jint i = 0; i < 1; i++) {
      ...
      OrgJodaTimeFormatDateTimeFormatterBuilder_TimeZoneId_initWithNSString_withInt_(e, names[i], i);
    }
    JreStrongAssignAndConsume(&OrgJodaTimeFormatDateTimeFormatterBuilder_TimeZoneId_BASE_GROUPED_IDS, new_JavaUtilArrayList_init());
    {
      ...
      for (NSString * __strong id_ in OrgJodaTimeFormatDateTimeFormatterBuilder_TimeZoneId_ALL_IDS) {
        jint pos = [((NSString *) nil_chk(id_)) indexOf:'/'];
        if (pos >= 0) {
          if (pos < ((jint) [id_ length])) {
            pos++;
          }
          maxPrefix = JavaLangMath_maxWithInt_withInt_(maxPrefix, pos);
          NSString *prefix = [id_ substring:0 endIndex:pos + 1];
          NSString *suffix = [id_ substring:pos];
        ...
        max = JavaLangMath_maxWithInt_withInt_(max, ((jint) [id_ length]));
      }
