...
//  source: /Users/marcussmith/Laundryman/Libraries/joda-time/src/main/java/org/joda/time/format/FormatUtils.java
//
...
inline jdouble OrgJodaTimeFormatFormatUtils_get_LOG_10(void);
...
__attribute__((unused)) static OrgJodaTimeFormatFormatUtils *new_OrgJodaTimeFormatFormatUtils_init(void) NS_RETURNS_RETAINED;

__attribute__((unused)) static OrgJodaTimeFormatFormatUtils *create_OrgJodaTimeFormatFormatUtils_init(void);
...
NSString *OrgJodaTimeFormatFormatUtils_createErrorMessageWithNSString_withInt_(NSString *text, jint errorPos) {
  ...
  NSString *sampleText;
  if ([((NSString *) nil_chk(text)) java_length] <= sampleLen + 3) {
    sampleText = text;
  }
  else {
    sampleText = [((NSString *) nil_chk([text java_substring:0 endIndex:sampleLen])) java_concat:@"..."];
  }
  if (errorPos <= 0) {
    return JreStrcat("$$C", @"Invalid format: \"", sampleText, '"');
  }
  if (errorPos >= [text java_length]) {
    return JreStrcat("$$$", @"Invalid format: \"", sampleText, @"\" is too short");
  }
  return JreStrcat("$$$$C", @"Invalid format: \"", sampleText, @"\" is malformed at \"", [((NSString *) nil_chk(sampleText)) java_substring:errorPos], '"');
}
