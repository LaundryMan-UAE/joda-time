...
//  source: /Users/marcussmith/Laundryman/Libraries/joda-time/src/main/java/org/joda/time/format/DateTimeParserBucket.java
//
...
__attribute__((unused)) static IOSObjectArray *OrgJodaTimeFormatDateTimeParserBucket__Annotations$0(void);

__attribute__((unused)) static IOSObjectArray *OrgJodaTimeFormatDateTimeParserBucket__Annotations$1(void);

__attribute__((unused)) static IOSObjectArray *OrgJodaTimeFormatDateTimeParserBucket__Annotations$2(void);

__attribute__((unused)) static IOSObjectArray *OrgJodaTimeFormatDateTimeParserBucket__Annotations$3(void);

__attribute__((unused)) static IOSObjectArray *OrgJodaTimeFormatDateTimeParserBucket__Annotations$4(void);
...
- (jlong)doParseMillisWithOrgJodaTimeFormatInternalParser:(id<OrgJodaTimeFormatInternalParser>)parser
  ...
  if (newPos >= 0) {
    if (newPos >= [((id<JavaLangCharSequence>) nil_chk(text)) java_length]) {
...
- (jboolean)restoreStateWithId:(id)savedState {
  if ([savedState isKindOfClass:[OrgJodaTimeFormatDateTimeParserBucket_SavedState class]]) {
    if ([((OrgJodaTimeFormatDateTimeParserBucket_SavedState *) nil_chk(((OrgJodaTimeFormatDateTimeParserBucket_SavedState *) savedState))) restoreStateWithOrgJodaTimeFormatDateTimeParserBucket:self]) {
...
- (jlong)computeMillisWithBoolean:(jboolean)resetFields
  ...
  if (iSavedFieldsShared_) {
    JreStrongAssign(&iSavedFields_, savedFields = [((IOSObjectArray *) nil_chk(iSavedFields_)) java_clone]);
    iSavedFieldsShared_ = false;
  }
  ...
  @try {
    ...
    if (resetFields) {
      for (jint i = 0; i < count; i++) {
        if (![((OrgJodaTimeDateTimeField *) nil_chk(((OrgJodaTimeFormatDateTimeParserBucket_SavedField *) nil_chk(IOSObjectArray_Get(nil_chk(savedFields), i)))->iField_)) isLenient]) {
          millis = [((OrgJodaTimeFormatDateTimeParserBucket_SavedField *) nil_chk(IOSObjectArray_Get(savedFields, i))) setWithLong:millis withBoolean:i == (count - 1)];
        }
      }
    }
  }
  @catch (OrgJodaTimeIllegalFieldValueException *e) {
    if (text != nil) {
      [e prependMessageWithNSString:JreStrcat("$@C", @"Cannot parse \"", text, '"')];
    }
    @throw e;
  }
  if (iOffset_ != nil) {
