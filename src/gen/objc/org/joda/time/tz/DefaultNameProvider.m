...
//  source: /Users/marcussmith/Laundryman/Libraries/joda-time/src/main/java/org/joda/time/tz/DefaultNameProvider.java
//
...
IOSObjectArray *OrgJodaTimeTzDefaultNameProvider_getNameSetWithJavaUtilLocale_withNSString_withNSString_withBoolean_(OrgJodaTimeTzDefaultNameProvider *self, JavaUtilLocale *locale, NSString *id_, NSString *nameKey, jboolean standardTime) {
  @synchronized(self) {
    ...
    if ([id_ java_hasPrefix:@"Etc/"]) {
      id_ = [id_ java_substring:4];
    }
    id<JavaUtilMap> byIdCache = [((JavaUtilHashMap *) nil_chk(self->iByLocaleCache2_)) getWithId:locale];
    if (byIdCache == nil) {
      [((JavaUtilHashMap *) nil_chk(self->iByLocaleCache2_)) putWithId:locale withId:byIdCache = OrgJodaTimeTzDefaultNameProvider_createCache(self)];
    }
    id<JavaUtilMap> byNameKeyCache = [((id<JavaUtilMap>) nil_chk(byIdCache)) getWithId:id_];
    if (byNameKeyCache == nil) {
      [byIdCache putWithId:id_ withId:byNameKeyCache = OrgJodaTimeTzDefaultNameProvider_createCache(self)];
    ...
    return JreRetainedLocalValue((IOSObjectArray *) cast_check([((id<JavaUtilMap>) nil_chk(byNameKeyCache)) getWithId:JavaLangBoolean_valueOfWithBoolean_(standardTime)], IOSClass_arrayType(NSString_class_(), 1)));
  }
}
