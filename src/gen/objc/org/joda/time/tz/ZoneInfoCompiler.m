...
//  source: /Users/marcussmith/Laundryman/Libraries/joda-time/src/main/java/org/joda/time/tz/ZoneInfoCompiler.java
//
...
__attribute__((unused)) static NSString *OrgJodaTimeTzZoneInfoCompiler_Rule_formatNameWithNSString_withInt_withNSString_(NSString *nameFormat, jint saveMillis, NSString *letterS);
...
- (id<JavaUtilMap>)compileWithJavaIoFile:(JavaIoFile *)outputDir
  ...
  if (sources != nil) {
    for (jint i = 0; i < sources->size_; i++) {
      ...
      @try {
        in = create_JavaIoBufferedReader_initWithJavaIoReader_(create_JavaIoFileReader_initWithJavaIoFile_(IOSObjectArray_Get(sources, i)));
  ...
  for (jint i = 0; i < [((id<JavaUtilList>) nil_chk(iGoodLinks_)) size]; i += 2) {
    ...
    if (sourceZone == nil) {
      [((JavaIoPrintStream *) nil_chk(JreLoadStatic(JavaLangSystem, out))) printlnWithNSString:JreStrcat("$$$$$", @"Cannot find source zone '", baseId, @"' to link alias '", alias, @"' to")];
    }
    else {
      OrgJodaTimeTzDateTimeZoneBuilder *builder = create_OrgJodaTimeTzDateTimeZoneBuilder_init();
      ...
      if (OrgJodaTimeTzZoneInfoLogger_verbose()) {
        [((JavaIoPrintStream *) nil_chk(JreLoadStatic(JavaLangSystem, out))) printlnWithNSString:JreStrcat("$$$$$", @"Good link: ", alias, @" -> ", baseId, @" revived")];
      }
    }
  }
  for (jint pass = 0; pass < 2; pass++) {
    for (jint i = 0; i < [((id<JavaUtilList>) nil_chk(iBackLinks_)) size]; i += 2) {
      ...
      if (tz == nil) {
        if (pass > 0) {
          [((JavaIoPrintStream *) nil_chk(JreLoadStatic(JavaLangSystem, out))) printlnWithNSString:JreStrcat("$$$$$", @"Cannot find time zone '", id_, @"' to link alias '", alias, @"' to")];
        }
      }
      else {
        [map putWithId:alias withId:tz];
        if (OrgJodaTimeTzZoneInfoLogger_verbose()) {
          [((JavaIoPrintStream *) nil_chk(JreLoadStatic(JavaLangSystem, out))) printlnWithNSString:JreStrcat("$$$$", @"Back link: ", alias, @" -> ", [tz getID])];
        }
      }
    }
  }
  if (outputDir != nil) {
    [((JavaIoPrintStream *) nil_chk(JreLoadStatic(JavaLangSystem, out))) printlnWithNSString:@"Writing ZoneInfoMap"];
  ...
  while ((line = [((JavaIoBufferedReader *) nil_chk(inArg)) readLine]) != nil) {
    ...
    if ([st hasMoreTokens]) {
      ...
      if ([((NSString *) nil_chk(token)) java_equalsIgnoreCase:@"Rule"]) {
        ...
        if (backward || [((NSString *) nil_chk(alias)) isEqual:@"US/Pacific-New"] || [alias java_hasPrefix:@"Etc/"] || [alias isEqual:@"GMT"]) {
...
void OrgJodaTimeTzZoneInfoCompiler_printUsage() {
  ...
  [((JavaIoPrintStream *) nil_chk(JreLoadStatic(JavaLangSystem, out))) printlnWithNSString:@"where possible options include:"];
  [((JavaIoPrintStream *) nil_chk(JreLoadStatic(JavaLangSystem, out))) printlnWithNSString:@"  -src <directory>    Specify where to read source files"];
  [((JavaIoPrintStream *) nil_chk(JreLoadStatic(JavaLangSystem, out))) printlnWithNSString:@"  -dst <directory>    Specify where to write generated files"];
  [((JavaIoPrintStream *) nil_chk(JreLoadStatic(JavaLangSystem, out))) printlnWithNSString:@"  -verbose            Output verbosely (default false)"];
}
...
void OrgJodaTimeTzZoneInfoCompiler_writeZoneInfoMapWithJavaIoDataOutputStream_withJavaUtilMap_(JavaIoDataOutputStream *dout, id<JavaUtilMap> zimap) {
  OrgJodaTimeTzZoneInfoCompiler_initialize();
  if (dout == nil) {
    @throw create_JavaLangIllegalArgumentException_initWithNSString_(@"DataOutputStream must not be null.");
  }
  ...
  [dout writeShortWithInt:[indexToId size]];
...
jint OrgJodaTimeTzZoneInfoCompiler_parseYearWithNSString_withInt_(NSString *str, jint def) {
  OrgJodaTimeTzZoneInfoCompiler_initialize();
  str = [((NSString *) nil_chk(str)) java_lowercaseStringWithJRELocale:JreLoadStatic(JavaUtilLocale, ENGLISH)];
...
jboolean OrgJodaTimeTzZoneInfoCompiler_testWithNSString_withOrgJodaTimeDateTimeZone_(NSString *id_, OrgJodaTimeDateTimeZone *tz) {
  ...
  jint stdOffset = [tz getStandardOffsetWithLong:millis];
  ...
  while (true) {
    ...
    jint nextStdOffset = [tz getStandardOffsetWithLong:millis];
    NSString *nextKey = [tz getNameKeyWithLong:millis];
    if (offset == nextOffset && stdOffset == nextStdOffset && [((NSString *) nil_chk(key)) isEqual:nextKey]) {
      ...
      if ([st hasMoreTokens]) {
        ...
        if ([str isEqual:@"24:00"]) {
          if (month == 12 && day == 31) {
            ...
            if (dayOfWeek != 0) {
              dayOfWeek = (JreIntMod((dayOfWeek - 1 + 1), 7)) + 1;
            }
          }
        }
        else {
...
- (void)addRecurringWithOrgJodaTimeTzDateTimeZoneBuilder:(OrgJodaTimeTzDateTimeZoneBuilder *)builder
                                                 withInt:(jint)negativeSave
                                            withNSString:(NSString *)nameFormat {
  jint saveMillis = iSaveMillis_ + -negativeSave;
  NSString *nameKey = OrgJodaTimeTzZoneInfoCompiler_Rule_formatNameWithNSString_withInt_withNSString_(nameFormat, saveMillis, iLetterS_);
  [((OrgJodaTimeTzZoneInfoCompiler_DateTimeOfYear *) nil_chk(iDateTimeOfYear_)) addRecurringWithOrgJodaTimeTzDateTimeZoneBuilder:builder withNSString:nameKey withInt:saveMillis withInt:iFromYear_ withInt:iToYear_];
}

+ (NSString *)formatNameWithNSString:(NSString *)nameFormat
                             withInt:(jint)saveMillis
                        withNSString:(NSString *)letterS {
  return OrgJodaTimeTzZoneInfoCompiler_Rule_formatNameWithNSString_withInt_withNSString_(nameFormat, saveMillis, letterS);
}
...
+ (const J2ObjcClassInfo *)__metadata {
  ...
  methods[1].selector = @selector(addRecurringWithOrgJodaTimeTzDateTimeZoneBuilder:withInt:withNSString:);
  methods[2].selector = @selector(formatNameWithNSString:withInt:withNSString:);
  ...
  static const void *ptrTable[] = { "LJavaUtilStringTokenizer;", "addRecurring", "LOrgJodaTimeTzDateTimeZoneBuilder;ILNSString;", "formatName", "LNSString;ILNSString;", "toString", "LOrgJodaTimeTzZoneInfoCompiler;" };
...
void OrgJodaTimeTzZoneInfoCompiler_Rule_initWithJavaUtilStringTokenizer_(OrgJodaTimeTzZoneInfoCompiler_Rule *self, JavaUtilStringTokenizer *st) {
  ...
  JreStrongAssign(&self->iName_, [((NSString *) nil_chk([st nextToken])) java_intern]);
...
NSString *OrgJodaTimeTzZoneInfoCompiler_Rule_formatNameWithNSString_withInt_withNSString_(NSString *nameFormat, jint saveMillis, NSString *letterS) {
  OrgJodaTimeTzZoneInfoCompiler_Rule_initialize();
  ...
  if (letterS == nil) {
    ...
    name = JreStrcat("$$$", left, letterS, right);
  }
...
- (void)addRecurringWithOrgJodaTimeTzDateTimeZoneBuilder:(OrgJodaTimeTzDateTimeZoneBuilder *)builder
  ...
  for (jint i = 0; i < [((id<JavaUtilList>) nil_chk(iRules_)) size]; i++) {
    ...
    if (((OrgJodaTimeTzZoneInfoCompiler_Rule *) nil_chk(rule))->iSaveMillis_ < 0) {
      negativeSave = JavaLangMath_minWithInt_withInt_(negativeSave, rule->iSaveMillis_);
    }
  }
  if (negativeSave < 0) {
    standardMillis += negativeSave;
    jint slashPos = [((NSString *) nil_chk(nameFormat)) java_indexOfString:@"/"];
    if (slashPos > 0) {
      nameFormat = JreStrcat("$C$", [nameFormat java_substring:slashPos + 1], '/', [nameFormat java_substring:0 endIndex:slashPos]);
    }
  }
  [((OrgJodaTimeTzDateTimeZoneBuilder *) nil_chk(builder)) setStandardOffsetWithInt:standardMillis];
  for (jint i = 0; i < [((id<JavaUtilList>) nil_chk(iRules_)) size]; i++) {
    OrgJodaTimeTzZoneInfoCompiler_Rule *rule = [((id<JavaUtilList>) nil_chk(iRules_)) getWithInt:i];
    [((OrgJodaTimeTzZoneInfoCompiler_Rule *) nil_chk(rule)) addRecurringWithOrgJodaTimeTzDateTimeZoneBuilder:builder withInt:negativeSave withNSString:nameFormat];
  }
}
...
+ (const J2ObjcClassInfo *)__metadata {
  ...
  methods[2].selector = @selector(addRecurringWithOrgJodaTimeTzDateTimeZoneBuilder:withInt:withNSString:);
  #pragma clang diagnostic pop
  static const J2ObjcFieldInfo fields[] = {
    { "iRules_", "LJavaUtilList;", .constantValue.asLong = 0, 0x2, -1, -1, 4, -1 },
  };
  static const void *ptrTable[] = { "LOrgJodaTimeTzZoneInfoCompiler_Rule;", "addRule", "addRecurring", "LOrgJodaTimeTzDateTimeZoneBuilder;ILNSString;", "Ljava/util/List<Lorg/joda/time/tz/ZoneInfoCompiler$Rule;>;", "LOrgJodaTimeTzZoneInfoCompiler;" };
  ...
  for (; zone != nil; zone = zone->iNext_) {
    if (zone->iRules_ == nil) {
      ...
      @catch (JavaLangException *e) {
        ...
        [rs addRecurringWithOrgJodaTimeTzDateTimeZoneBuilder:builder withInt:zone->iOffsetMillis_ withNSString:zone->iFormat_];
      }
    }
